<h1 align="center">Vinicius de Miranda Melo</h1>

<p align="center">
  <b>Desenvolvedor Back-end · Java · Spring Boot · Spring Security</b><br>
  São Paulo, Brasil
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/vinicius-miranda-melo">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="mailto:viniciusmmelo03@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
</p>

---

## 👨‍💻 Sobre mim

Construo **APIs REST seguras, escaláveis e bem estruturadas** com Java e Spring Boot.

Nos meus projetos implementei autenticação **JWT com refresh token**, **two-factor authentication (TOTP)**, **login social com OAuth2** (Google e GitHub), **arquitetura de microsserviços** com Spring Cloud (Eureka, Gateway, OpenFeign) e **comunicação assíncrona orientada a eventos** com Apache Kafka.

Sou formado em **Análise e Desenvolvimento de Sistemas** e atuo hoje com suporte e infraestrutura de TI no Grupo Roveri — experiência que me deu base sólida em ambientes corporativos, redes e resolução de problemas. Direciono minha carreira para **desenvolvimento back-end**.

**Áreas de maior interesse:** segurança de aplicações, arquitetura de sistemas distribuídos e boas práticas de API design.

---

## 📌 Projetos em destaque

| Projeto | O que é | Destaques técnicos |
|---|---|---|
| **[Forum Hub](https://github.com/vinimiiranda/Forum)** | API REST de fórum com gestão de tópicos e respostas | JWT + refresh token, 2FA via TOTP, OAuth2 (Google/GitHub), testes de integração com Testcontainers, CI no GitHub Actions |
| **[Ecommerce Kafka](https://github.com/vinimiiranda/Ecommerce-kafka)** | E-commerce orientado a eventos com Apache Kafka | 6 serviços desacoplados, producers e consumers genéricos, detecção de fraude, consumo por regex |
| **[Food Microservices API](https://github.com/vinimiiranda/food-microservices-api)** | Plataforma de pedidos e pagamentos em microsserviços | Spring Cloud, Eureka, API Gateway, OpenFeign, load balancing |
| **[Clínica Médica API](https://github.com/vinimiiranda/clinica-medica-api)** | API REST de gestão de clínica médica | CRUD completo, Spring Security + JWT, Flyway, validações de agendamento |

---

## 🧰 Stack

**Backend**

<p>
  <img src="https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white" />
  <img src="https://img.shields.io/badge/Spring%20Boot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white" />
  <img src="https://img.shields.io/badge/Spring%20Security-6DB33F?style=for-the-badge&logo=springsecurity&logoColor=white" />
  <img src="https://img.shields.io/badge/Spring%20Cloud-6DB33F?style=for-the-badge&logo=spring&logoColor=white" />
  <img src="https://img.shields.io/badge/JPA%2FHibernate-59666C?style=for-the-badge&logo=hibernate&logoColor=white" />
  <img src="https://img.shields.io/badge/Apache%20Kafka-231F20?style=for-the-badge&logo=apachekafka&logoColor=white" />
</p>

**Banco de dados**

<p>
  <img src="https://img.shields.io/badge/MySQL-00758F?style=for-the-badge&logo=mysql&logoColor=white" />
  <img src="https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white" />
  <img src="https://img.shields.io/badge/Flyway-CC0200?style=for-the-badge&logo=flyway&logoColor=white" />
</p>

**Cloud, DevOps e testes**

<p>
  <img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white" />
  <img src="https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazonaws&logoColor=white" />
  <img src="https://img.shields.io/badge/GitHub%20Actions-2088FF?style=for-the-badge&logo=githubactions&logoColor=white" />
  <img src="https://img.shields.io/badge/Testcontainers-291A3F?style=for-the-badge&logo=testcontainers&logoColor=white" />
</p>

**Ferramentas**

<p>
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white" />
  <img src="https://img.shields.io/badge/Maven-C71A36?style=for-the-badge&logo=apachemaven&logoColor=white" />
  <img src="https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white" />
  <img src="https://img.shields.io/badge/IntelliJ-000000?style=for-the-badge&logo=intellijidea&logoColor=white" />
</p>

---

## 🎯 Em evolução

Aprofundando **deploy em AWS** e observabilidade de sistemas distribuídos.

---

<p align="center">
  📫 Aberto a oportunidades como Desenvolvedor Back-end Júnior — <a href="mailto:viniciusmmelo03@gmail.com">viniciusmmelo03@gmail.com</a>
</p>
